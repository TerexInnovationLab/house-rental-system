<aside class="dashboard-sidebar w-64 bg-white shadow-lg fixed top-16 h-[calc(100vh-4rem)] transition-transform duration-300 z-50 overflow-y-auto">
    ...
    <nav class="p-4">
        <a href="{{ route('admin.dashboard') }}" class="sidebar-link js-nav-link flex items-center gap-3 px-4 py-3 rounded-lg mb-2 {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}">
            <span class="font-medium">Overview</span>
        </a>
        <a href="{{ route('admin.users') }}" class="sidebar-link js-nav-link flex items-center gap-3 px-4 py-3 rounded-lg mb-2 {{ request()->routeIs('admin.users') ? 'active' : '' }} text-gray-700">
            <span class="font-medium">Users</span>
            <span class="ml-auto bg-purple-100 text-purple-700 text-xs px-2 py-1 rounded-full">2.4K</span>
        </a>
        <a href="{{ route('admin.properties') }}" class="sidebar-link js-nav-link flex items-center gap-3 px-4 py-3 rounded-lg mb-2 {{ request()->routeIs('admin.properties') ? 'active' : '' }} text-gray-700">
        ...
        <a href="{{ route('admin.disputes') }}" class="sidebar-link js-nav-link flex items-center gap-3 px-4 py-3 rounded-lg mb-2 {{ request()->routeIs('admin.disputes') ? 'active' : '' }} text-gray-700">
